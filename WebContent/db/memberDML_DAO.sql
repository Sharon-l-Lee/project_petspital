...
--이전글
SELECT A.* FROM(SELECT FNUM, FSUBJECT, LAG(FNUM,1) OVER(ORDER BY fGROUP DESC, FSTEP) AS PREV, LAG(FSUBJECT,1,'이전글이 없습니다')
                    OVER( ORDER BY FNUM) AS PREV_TITLE
             FROM FILEBOARD)A
                WHERE FNUM=4;
    
SELECT FNUM, FSUBJECT, LAG(FNUM,1) OVER(ORDER BY fGROUP DESC, FSTEP) AS PREV, LAG(FSUBJECT,1,'이전글이 없습니다')
    OVER( ORDER BY FNUM) AS PREV_TITLE
    FROM FILEBOARD;
    
--다음글
SELECT A.* FROM (SELECT FNUM, FSUBJECT, LEAD(FNUM,1) OVER(ORDER BY fGROUP DESC, FSTEP) AS PREV, LEAD(FSUBJECT,1,'다음글이 없습니다')
    OVER( ORDER BY FNUM) AS NEXT_TITLE
    FROM FILEBOARD)A
    WHERE FNUM=4;
SELECT FNUM, FSUBJECT, LEAD(FNUM,1) OVER(ORDER BY fGROUP DESC, FSTEP) AS PREV, LEAD(FSUBJECT,1,'다음글이 없습니다')
    OVER( ORDER BY FNUM) AS NEXT_TITLE
    FROM FILEBOARD;
