...
//	    VALUES(NOTICE_SEQ.NEXTVAL, 'admin1', '글1', '글1입니다', 'noImg.png', '127.10.26');
//
	public int writeNboard(String aid, String nsubject, String ncontent, String nfilename, String nip ) {
		...
		try { //원글쓰기는 step과 레벨 모두 0고정
			...
			System.out.println("공지 글쓰기 성공");
		} catch (SQLException e) {
			System.out.println(e.getMessage()+"공지 글쓰기실패");
		} finally {
...
//	                WHERE nNUM ='1';
	public int modifyNBoard(String nsubject, String ncontent, String nfilename, int nnum){
		...
		try {
			...
			pstmt.setInt(4, nnum);
			result = pstmt.executeUpdate();
			System.out.println(result ==SUCCESS ? "공지 수정 성공" : "공지 수정실패");
		} catch (SQLException e) {
...
//
	public int deleteNBoard(int nnum){
		...
		try { 
			...
			System.out.println(e.getMessage()+"공지 삭제실패");
		} finally {
